...
#CSV_PATH=os.path.join('./dat/', "ENIGMA_Sev.YBOC.csv") : Severity 5개 class

# ------------------------------------전체 data---------------------------------------
# 보정 안된 전체 data : All_보정X_total.PNG
#CSV_PATH=os.path.join('./dat/Sev_data/', "ENIGMA_Sev_YBOC(total).csv")

# 보정된 전체 data : All_보정O_total.PNG
#CSV_PATH=os.path.join('./dat/Sev_data/', "ENIGMA_Sev_YBOC_adjusted(total).csv")


# ------------------------------------진단 data---------------------------------------
# 진단된 그룹 - 보정 x data : Dx_보정X_data.PNG
#CSV_PATH=os.path.join('./dat/Sev_data/', "ENIGMA_Sev_YBOC_Dx1(total).csv")

# 진단된 그룹 - 보정 data : Dx_보정O_data.PNG
#CSV_PATH=os.path.join('./dat/Sev_data/', "ENIGMA_Sec_YBOC_adjusted_Dx1(total).csv")


# -----------------------보정 o & 전체/진단 data 내 adult & pediatric-----------------
# 보정o 진단된 data - adult data : Dx_보정O_adult.PNG
#CSV_PATH=os.path.join('./dat/Sev_data/', "ENIGMA_Sev_YBOC_adjusted_Dx1(adult).csv")

# 보정o 진단된 data - pediatric data : Dx_보정O_pediatric.PNG
#CSV_PATH=os.path.join('./dat/Sev_data/', "ENIGMA_Sev_YBOC_adjusted_Dx1(pediatric).csv")

# 보정o 전체 그룹 data - adult data : All_보정O_adult.PNG
#CSV_PATH=os.path.join('./dat/Sev_data/', "ENIGMA_Sev_YBOC_adjusted(adult).csv")

# 보정o 전체 그룹 data - pediatric data : All_보정O_pediatric.PNG
#CSV_PATH=os.path.join('./dat/Sev_data/', "ENIGMA_Sev_YBOC_adjusted(pediatric).csv")


# -----------------------보정 x & 전체/진단 data 내 adult & pediatric-----------------
# 보정x 진단된 data - adult data : Dx_보정X_adult.PNG
#CSV_PATH=os.path.join('./dat/Sev_data/', "ENIGMA_Sev_YBOC_Dx1(adult).csv")

# 보정x 진단된 data - pediatric data : Dx_보정X_pediatric.PNG
#CSV_PATH=os.path.join('./dat/Sev_data/', "ENIGMA_Sev_YBOC_Dx1(pediatric).csv")

# 보정x 전체 그룹 data - adult data : All_보정X_adult.PNG
#CSV_PATH=os.path.join('./dat/Sev_data/', "ENIGMA_Sev_YBOC(adult).csv")

# 보정x 전체 그룹 data - pediatric data : All_보정X_pediatric.PNG
#CSV_PATH=os.path.join('./dat/Sev_data/', "ENIGMA_Sev_YBOC(pediatric).csv")
...
def create_data_set(complete=False, completeness_threshold=0.9, age_group=None, covariates=None, query_str=None,
    ...
    # 보정x data에서 col 삭제
    #df = df.drop(['Sev.scr20','Sev.scr15_23','Sev.50per','Sev.304030per', 'Sev.score'],axis='columns')
    
    # 보정o data에서 col 삭제
    df = df.drop(['missing.value','missing_ICV','total_ICV','total_ICV.3', 'Sev.score'],axis='columns')
    

    print('Loading ENIGMA dataset: Complete = {}, Min_Threshold = {}, Age_Group = {}, Covariates = {}, ''Min_Counts_per_Site = {}, y_label = {} \n'.format(complete, completeness_threshold, age_group, covariates, min_counts_per_site, y_label))
    ...
    #print('subcort_idx:{},{}'.format(first_fs_index,np.where(df.columns.values == 'subcort_ICV')))
    ...
    fs_labels = df.columns.values[range(first_fs_index, len(df.columns) - 1)]
    #cov_labels = np.append(df.columns.values[range(0, first_fs_index)], ["age_group_tesla_site", "Dx"])
    cov_labels = np.append(df.columns.values[range(0, first_fs_index)], ["age_group_tesla_site", "Sev.score.2"])
    age_group_labels = np.unique(df.age_group)

    #y_label = y_label if y_label else "Dx"
    y_label = y_label if y_label else "Sev.score.2"
    ...
    if verbose:
        print('Features to extract ({}/{}): \n {} \n'.format(len(all_selected_features), len(fs_labels) + len(cov_labels), all_selected_features))
